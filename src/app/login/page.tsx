...
export default function LoginPage() {
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    try {
      // Simulate network delay to show off the loading animation
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative">
      <div className="relative z-10 w-full max-w-md">
        <Link href="/" className="text-white/90 font-medium hover:text-white hover:underline text-sm flex items-center gap-1 mb-8 drop-shadow-md">
        ...
        <div className="w-full bg-white/10 backdrop-blur-3xl rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.2)] border border-white/20 overflow-hidden animate-entrance group">
          ...
          <div className="px-8 pt-10 pb-6 text-center relative z-10">
            ...
            <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-300 drop-shadow-md pb-1">Login Dashboard</h1>
            <p className="text-white/80 text-sm mt-1 font-medium">Masuk untuk mengelola pendaftaran dan mengumpulkan karya</p>
          </div>

          <form className="px-8 pb-8 space-y-6 relative z-10" onSubmit={(e) => { e.preventDefault(); handleLogin(e); }}>
            {error && (
            ...
            <Button type="submit" disabled={isLoading} className="w-full py-4 bg-white/20 border border-white/30 backdrop-blur-md shadow-[0_10px_20px_rgba(0,0,0,0.1)] hover:-translate-y-1 hover:bg-white/30 text-white rounded-[1.2rem] font-bold transition-all duration-300">
              {isLoading ? (
            ...
            <div className="text-center mt-6">
              <p className="text-sm text-white/70 font-medium">
                Belum punya akun? <Link href="/pendaftaran" className="text-white font-bold hover:underline transition-colors drop-shadow-sm">Daftar sekarang</Link>
              </p>
            </div>

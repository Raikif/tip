...
import { Check, ChevronRight, FileImage, CreditCard, Lock, Info } from "lucide-react";
...
export default function RegistrationPage() {
  ...
  const uploadFile = async (file: File, field: string): Promise<{ url: string; fileName: string; fileSize: number; fileType: string }> => {
    const payload = new FormData();
    payload.append("file", file);
    payload.append("teamName", formData.teamName || "unknown");
    payload.append("field", field);

    console.log("[CLIENT] Uploading file:", file.name, "field:", field);
    ...
    console.log("[CLIENT] Upload response status:", res.status, "body:", text);
    let json: any = {};
    try { json = JSON.parse(text); } catch { /* ignore */ }

    if (!res.ok || !json?.ok) {
      throw new Error(json?.error || `Upload ${field} gagal.`);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const message = uploadErr instanceof Error ? uploadErr.message : "Gagal upload file.";
        setError(message);
        setIsLoading(false);
        return;
      }

      console.log("[CLIENT] Submitting registration to /api/registrations");
      ...
      const text = await res.text();
      console.log("[CLIENT] Registration response status:", res.status, "body:", text);
      let json: any = {};
      try { json = JSON.parse(text); } catch { /* ignore */ }

      if (!res.ok || !json?.ok) {
        setError(json?.error || "Terjadi kesalahan saat submit.");
        setIsLoading(false);
        return;
      }
  ...
  if (isTimeLocked) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-3xl font-bold text-slate-800 mb-4">Pendaftaran Belum Dibuka / Telah Ditutup</h1>
        <p className="text-slate-600 max-w-md">
          Pendaftaran dibuka mulai 21 Juli 2026 hingga 31 Agustus 2026. Silakan periksa kembali jadwal kegiatan (Timeline).
        </p>
        <Link href="/">
  ...
  const FileUploadBox = ({ title, desc }: { title: string; desc: string }) => (
    <div className="border-2 border-dashed border-purple-300/60 bg-white/50 rounded-2xl p-4 text-center hover:bg-purple-50/50 hover:border-[#B44DFF] transition-all duration-300 cursor-pointer group mt-2 shadow-sm">
      ...
      <p className="text-sm font-bold text-slate-800 group-hover:text-[#B44DFF] transition-colors">{title}</p>
            ...
            {steps.map((step) => (
              <React.Fragment key={step.id}>
                ...
                {step.id !== steps.length && (
                  <div className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${step.id < currentStep ? "bg-white/30" : "bg-black/10"}`}></div>
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 md:p-10 relative">
          {currentStep === 1 && (
            <div className="space-y-6 animate-entrance relative z-10">
              ...
              <div className="space-y-5 mt-6">
                <Input name="teamName" value={formData.teamName} onChange={handleChange} label="Nama Tim" placeholder="Masukkan nama tim" variant="glass" required />
                <Input name="institution" value={formData.institution} onChange={handleChange} label="Asal Instansi / Sekolah / Universitas" placeholder="Contoh: Universitas Gadjah Mada" variant="glass" required />
              </div>
            </div>
          )}
            ...
            <div className="space-y-12 animate-entrance relative z-10">
              ...
              <div className="space-y-5 bg-black/10 p-6 rounded-[1.5rem] border border-white/10 backdrop-blur-sm">
                <h2 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-300 flex items-center gap-3 drop-shadow-sm pb-1">
                  <div className="w-10 h-10 rounded-[0.8rem] bg-white/20 border border-white/30 text-white flex items-center justify-center text-sm shadow-inner backdrop-blur-md">1</div>
                  Data Ketua Tim (Wajib)
                </h2>
                <div className="space-y-5 mt-6">
                  <Input name="leaderName" value={formData.leaderName} onChange={handleChange} label="Nama Lengkap Ketua" placeholder="Sesuai Kartu Pelajar / KTM" variant="glass" required />
                  <Input name="leaderNim" value={formData.leaderNim} onChange={handleChange} label="NIM / NISN / NIS" placeholder="Masukkan Nomor Induk" variant="glass" required />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <Input name="leaderWa" value={formData.leaderWa} onChange={handleChange} label="Nomor WhatsApp" type="tel" placeholder="Contoh: 08123456789" variant="glass" required />
                    <Input name="leaderEmail" value={formData.leaderEmail} onChange={handleChange} label="Email Aktif" type="email" placeholder="email@contoh.com" variant="glass" required />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <Input name="leaderPassword" value={formData.leaderPassword} onChange={handleChange} label="Kata Sandi (Password)" type="password" placeholder="Untuk login ke Dashboard" variant="glass" required />
                    <Input label="Konfirmasi Kata Sandi" type="password" placeholder="Ulangi password" variant="glass" required />
                  </div>
                  <div
                    ...
                    tabIndex={0}
                    onKeyDown={(ev) => {
                      if (ev.key === "Enter" || ev.key === " ") ketuaKtmRef.current?.click();
                    }}
                  >
                    ...
                    <p className="text-xs text-white/60 mt-1 font-medium">Ketua Tim (PDF/JPG, maks. 2MB)</p>
                    {files.ketuaKtm && (
                      <p className="text-xs text-green-300 mt-2 font-medium">
                        Dipilih: {files.ketuaKtm.name}
                      </p>
                    )}
                    <input ref={ketuaKtmRef} type="file" className="hidden" accept="image/*,.pdf" onChange={handleFileChange("ketuaKtm")} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-5 px-2">
                <h2 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-300 to-purple-300 flex items-center justify-between drop-shadow-sm pb-1">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-[0.8rem] bg-white/10 border border-white/10 text-white/70 flex items-center justify-center text-sm shadow-inner">2</div>
                    Data Anggota 1
                  </div>
                  <span className="text-[10px] font-bold bg-white/10 text-white/70 border border-white/10 px-3 py-1.5 rounded-full uppercase tracking-widest backdrop-blur-sm">Opsional</span>
                </h2>
                <div className="space-y-5 mt-4">
                  <Input name="member1Name" value={formData.member1Name} onChange={handleChange} label="Nama Lengkap Anggota 1" placeholder="Kosongkan jika tidak ada" variant="glass" />
                  <Input name="member1Nim" value={formData.member1Nim} onChange={handleChange} label="NIM / NISN / NIS Anggota 1" placeholder="Kosongkan jika tidak ada" variant="glass" />
                  <div
                    ...
                    tabIndex={0}
                    onKeyDown={(ev) => {
                      if (ev.key === "Enter" || ev.key === " ") anggota1Ref.current?.click();
                    }}
                  >
                    ...
                    <p className="text-xs text-white/50 mt-1 font-medium">Anggota 1 (Opsional)</p>
                    {files.anggota1Ktm && (
                      <p className="text-xs text-green-300 mt-2 font-medium">
                        Dipilih: {files.anggota1Ktm.name}
                      </p>
                    )}
                    <input ref={anggota1Ref} type="file" className="hidden" accept="image/*,.pdf" onChange={handleFileChange("anggota1Ktm")} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-5 border-t border-white/10 pt-8 px-2">
                <h2 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-pink-300 to-purple-300 flex items-center justify-between drop-shadow-sm pb-1">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-[0.8rem] bg-white/10 border border-white/10 text-white/70 flex items-center justify-center text-sm shadow-inner">3</div>
                    Data Anggota 2
                  </div>
                  <span className="text-[10px] font-bold bg-white/10 text-white/70 border border-white/10 px-3 py-1.5 rounded-full uppercase tracking-widest backdrop-blur-sm">Opsional</span>
                </h2>
                <div className="space-y-5 mt-4">
                  <Input name="member2Name" value={formData.member2Name} onChange={handleChange} label="Nama Lengkap Anggota 2" placeholder="Kosongkan jika tidak ada" variant="glass" />
                  <Input name="member2Nim" value={formData.member2Nim} onChange={handleChange} label="NIM / NISN / NIS Anggota 2" placeholder="Kosongkan jika tidak ada" variant="glass" />
                  <div
                    className="border border-dashed border-white/20 bg-white/5 rounded-[1.2rem] p-5 text-center hover:bg-white/10 hover:border-white/40 transition-all duration-300 cursor-pointer group mt-4 shadow-sm backdrop-blur-sm"
                    ...
                    tabIndex={0}
                    onKeyDown={(ev) => {
                      if (ev.key === "Enter" || ev.key === " ") anggota2Ref.current?.click();
                    }}
                  >
                    <div className="w-14 h-14 bg-white/10 text-white/80 rounded-[1rem] flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform shadow-inner border border-white/10">
                      <FileImage size={24} />
                    ...
                    <p className="text-xs text-white/50 mt-1 font-medium">Anggota 2 (Opsional)</p>
                    {files.anggota2Ktm && (
                      <p className="text-xs text-green-300 mt-2 font-medium">
                        Dipilih: {files.anggota2Ktm.name}
                      </p>
                    )}
                    <input ref={anggota2Ref} type="file" className="hidden" accept="image/*,.pdf" onChange={handleFileChange("anggota2Ktm")} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8 animate-entrance relative z-10">
              <div className="bg-white/10 backdrop-blur-2xl border border-white/20 p-8 rounded-[1.5rem] text-center shadow-[0_10px_30px_rgba(0,0,0,0.1)] relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-300 mb-2 drop-shadow-sm pb-1">Pembayaran Pendaftaran</h3>
                  <p className="text-white/80 mb-6 font-medium">Silakan lakukan transfer pendaftaran ke rekening berikut:</p>
                  <div className="bg-white/20 backdrop-blur-md px-6 py-4 rounded-[1.1rem] border border-white/30 inline-block shadow-inner mb-6 hover:bg-white/25 transition-all cursor-pointer">
                    <p className="font-extrabold text-3xl tracking-wider text-yellow-300 drop-shadow-md">BCA 1234567890</p>
                    <p className="text-sm text-white/80 font-bold mt-1">a.n. Tech Innovation Paper</p>
                  </div>
                  <div>
                    <p className="font-black text-white bg-white/20 border border-white/30 backdrop-blur-md inline-block px-5 py-2.5 rounded-full shadow-inner">
                      Nominal: <span className="text-lg ml-1">(Segera Diumumkan)</span>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-dashed border-white/40 bg-white/5 rounded-[1.5rem] p-12 text-center hover:bg-white/15 hover:border-white/60 transition-all duration-300 cursor-pointer group shadow-sm backdrop-blur-md">
                ...
                <p className="text-xl font-black text-cyan-300 group-hover:drop-shadow-md transition-all">Unggah Bukti Transaksi</p>
                <p className="text-sm text-white/60 mt-2 font-medium">Maks. 5MB (Format: .jpg, .png, .pdf)</p>
                <input type="file" className="hidden" accept="image/*,.pdf" />
              </div>

              <div className="mt-8 bg-black/10 p-5 rounded-[1.1rem] border border-white/10 backdrop-blur-sm">
                <label className="flex items-start gap-4 cursor-pointer">
                  <input type="checkbox" className="mt-1 w-6 h-6 rounded border-white/30 bg-white/10 text-white focus:ring-white/50" required />
          ...
          {currentStep === 4 && (
            <div className="space-y-6 animate-entrance text-center py-16 relative z-10">
              ...
              <h2 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-amber-400 mb-4 drop-shadow-md pb-1">Pendaftaran Berhasil!</h2>
              <p className="text-white/80 mb-12 max-w-md mx-auto leading-relaxed text-lg font-medium drop-shadow-sm">
                Data pendaftaranmu sedang diproses. Silakan masuk ke Dashboard untuk melihat status verifikasi.
              </p>
              <Link href="/login">
                <Button type="button" className="bg-white/20 text-white border border-white/40 hover:bg-white/30 backdrop-blur-md px-12 py-5 text-lg font-bold rounded-[1.5rem] transition-all duration-300 shadow-[0_10px_30px_rgba(0,0,0,0.1)] hover:-translate-y-1">
                  Masuk ke Dashboard
                </Button>
              </Link>
            </div>
